...
#
# The nested for loops scan every cell in the grid, which takes O(R * C).
#
# BFS does NOT make the complexity O((R*C)^2). Because we use the `visit`
# set, each land cell is added to the queue and processed by BFS at most once.
# When the outer loop later reaches land that BFS already discovered, it sees
# that the coordinate is already in `visit` and does not run BFS from it again.
#
# For every cell processed by BFS, we check exactly 4 directions
# (up, down, left, right). 4 is a constant, so this is still O(R * C).
#
# Therefore:
# grid scan O(R*C) + total BFS work O(R*C)
# = O(2*R*C)
# = O(R*C)
#
# IMPORTANT: Nested loops do NOT automatically mean O(n^2).
# Here one loop runs R times and the other C times, so it is O(R*C).
# If N represents the TOTAL number of cells in the grid, then R*C = N,
# meaning we could also describe the time complexity as O(N).


# SPACE COMPLEXITY: O(R * C)
#
# `visit` stores coordinates that we have already discovered.
# In the worst case (for example, if the entire grid is land), it can
# eventually contain all R*C coordinates -> O(R*C).
#
# BFS also uses a queue. The queue can hold multiple coordinates waiting
# to be processed. We can safely upper-bound its space by O(R*C).
#
# Therefore the overall auxiliary space complexity is O(R*C).
#
# If N = total number of cells, space can also be written as O(N).
