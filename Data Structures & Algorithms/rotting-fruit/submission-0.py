    ...
    def orangesRotting(self, grid: List[List[int]]) -> int:
        ...
        for r in range(rows):
            for c in range(cols):
                if grid[r][c] == 2:
                    q.append((r,c))
                    visit.add((r,c)) # adding the rotten fruit to the queue and the visit set to mark that we have visited it
                elif grid[r][c] == 1:
                    fresh += 1
