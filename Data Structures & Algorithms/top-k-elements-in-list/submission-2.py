class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        result = []
        for i in range(len(bucket)-1, -1 , -1): # start, stop, step so we are starting at the end of bucket and moving backwards by 1
            for num in bucket[i]: # this helps us check if there is a number in the current bucket we are in
            # if there isnt a number we skip this whole loop
                result.append(num)
